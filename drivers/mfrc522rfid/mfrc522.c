...
#define MFRC522_RST_PIN  PB0 //PB1
...
void mfrc522_init(void)
{
    ...
    mfrc522_antenna_on();
    uint8_t v = mfrc522_get_version();
    uart_write_hex_buffer(&v, 1);
}
...
mfrc522_status_t mfrc522_anticoll_select(mfrc522_uid_t *uid)
{
    ...
    mfrc522_status_t status =
        mfrc522_transceive(buffer, 2, buffer, &back_len, &valid_bits);

    if (status != MFRC522_OK){
        uart_write_string("uid?:");
        uart_write_string("\n");
        uart_write_hex_buffer(buffer, back_len);
        uart_write_string("\n");
        return status;
    }
    if (back_len != 5){
    ...
    if (bcc != buffer[4])
    uart_write_string("bcc!= buffer 4");
    uart_write_hex_buffer(buffer, back_len);
    uart_write_string("\n");
        return MFRC522_ERROR;
    uid->uid[0] = buffer[0];
    ...
    uart_write_string("uid?:");
    uart_write_string("\n");
    uart_write_hex_buffer(uid->uid, uid->size);
    uart_write_string("\n");
                                           ...
                                           uint8_t *valid_bits)
{
    ...
    if (mfrc522_read_reg(FIFOLevelReg) == 0)  //nytt ta bort? 
    {
        return MFRC522_ERROR;
    }
...
//     if (!uid)
//     {
//         uart_write_string("no uid found i anticol");
//         return MFRC522_ERROR;
//     }
...
//     if (status != MFRC522_OK)
//     {
//         uart_write_string("status ! ok ");
//         return status;
//     }

//     if (back_len != 5)
//     {
//         uart_write_string("back len ! 5");
//         return MFRC522_ERROR;
//     }
...
//     //     uart_write_string("\nexp bcc: ");
//     //     uart_write_hex_buffer(&buffer[4], back_len);
...
//     {
//         uart_write_string("mfrc_calculate ! ok");
//         return MFRC522_ERROR;
//     }
...
//         uart_write_string("\n");//test
//         uart_write_hex_buffer(uid -> uid, 4); //test
//         uart_write_string("\n");
    
//     if (status != MFRC522_OK)
//     {
//         uart_write_string("row 331 ! ok ");
//         return status;
//     }

//     if (back_len < 1)//(back_len != 3)
//     {
//         uart_write_string("back lan < 1! row 340");
//         return MFRC522_ERROR;
//     }
